 ...
 *
 * Flow:
 *   1. On invocation, check npm registry (cached 24h in ~/.caravo/version-check.json)
 *   2. If a newer version exists and we're running from npx cache, delete our cache entry
 *   3. Next `npx @caravo/cli` invocation re-downloads the latest version
 *   4. For global installs, show a notice suggesting `caravo update`
 *
...
export async function checkForUpdate(
  ...
  try {
    ...
    if (isNewer(latest, currentVersion)) {
      // Clear npx cache so next npx invocation gets the new version
